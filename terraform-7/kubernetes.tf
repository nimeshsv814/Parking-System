  ...
  services = {
    auth = {
      ...
      env = {
        ...
        CLOUDWATCH_LOG_GROUP = aws_cloudwatch_log_group.app_services["auth"].name
        JWT_EXPIRES_IN       = "7d"
        CORS_ORIGIN          = "*"
      }
    ...
    parking = {
      ...
      env = {
        ...
        CLOUDWATCH_LOG_GROUP = aws_cloudwatch_log_group.app_services["parking"].name
        CORS_ORIGIN          = "*"
      }
    ...
    booking = {
      ...
      env = {
        ...
        CLOUDWATCH_LOG_GROUP            = aws_cloudwatch_log_group.app_services["booking"].name
        CORS_ORIGIN                     = "*"
        PARKING_SERVICE_URL             = "http://parking-service:4002"
        NOTIFICATION_SERVICE_URL        = "http://notification-service:4006"
        BOOKING_HOLD_MINUTES            = "10"
        BOOKING_CONFIRMED_SNS_TOPIC_ARN = aws_sns_topic.booking_confirmed.arn
        BOOKING_CANCELLED_SNS_TOPIC_ARN = aws_sns_topic.booking_cancelled.arn
        SQS_NOTIFICATION_QUEUE_NAME     = aws_sqs_queue.notification.name
        SQS_NOTIFICATION_QUEUE_URL      = aws_sqs_queue.notification.url
      }
    ...
    payment = {
      ...
      env = {
        ...
        APP_CONFIG_SECRET_ARN       = aws_secretsmanager_secret.app_config.arn
        CLOUDWATCH_LOG_GROUP        = aws_cloudwatch_log_group.app_services["payment"].name
        CORS_ORIGIN                 = "*"
        BOOKING_SERVICE_URL         = "http://booking-service:4003"
        NOTIFICATION_SERVICE_URL    = "http://notification-service:4006"
        RAZORPAY_CURRENCY           = var.razorpay_currency
        PAYMENT_INVOICE_BUCKET      = aws_s3_bucket.payment_invoices.bucket
        PAYMENT_INVOICE_KMS_KEY_ARN = aws_kms_key.payment_invoices.arn
        SQS_NOTIFICATION_QUEUE_NAME = aws_sqs_queue.notification.name
        SQS_NOTIFICATION_QUEUE_URL  = aws_sqs_queue.notification.url
      }
    ...
    scheduler = {
      ...
      env = {
        ...
        BOOKING_TABLE               = var.booking_table
        APP_CONFIG_SECRET_ARN       = aws_secretsmanager_secret.app_config.arn
        CLOUDWATCH_LOG_GROUP        = aws_cloudwatch_log_group.app_services["scheduler"].name
        BOOKING_SERVICE_URL         = "http://booking-service:4003"
        CRON_SCHEDULE               = "* * * * *"
        SQS_NOTIFICATION_QUEUE_NAME = aws_sqs_queue.notification.name
        SQS_NOTIFICATION_QUEUE_URL  = aws_sqs_queue.notification.url
    ...
    notification = {
      ...
      env = {
        ...
        APP_CONFIG_SECRET_ARN           = aws_secretsmanager_secret.app_config.arn
        CLOUDWATCH_LOG_GROUP            = aws_cloudwatch_log_group.app_services["notification"].name
        CORS_ORIGIN                     = "*"
        SQS_NOTIFICATION_QUEUE_NAME     = aws_sqs_queue.notification.name
        SQS_NOTIFICATION_QUEUE_URL      = aws_sqs_queue.notification.url
        BOOKING_CONFIRMED_SNS_TOPIC_ARN = aws_sns_topic.booking_confirmed.arn
    ...
    frontend = {
      ...
      port  = 80
      env = {
        CLOUDWATCH_LOG_GROUP = aws_cloudwatch_log_group.app_services["frontend"].name
      }
      secret_env = []
    }
  }
