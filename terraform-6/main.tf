...
module "booking_sns_notifications" {
  ...
  enable_sqs_subscription             = var.enable_booking_sns_to_sqs_subscription && (var.create_sqs_notification_queue || (var.sqs_notification_queue_url != "" && var.sqs_notification_queue_arn != "" && length(regexall("\\*", var.sqs_notification_queue_arn)) == 0))
  notification_queue_arn              = local.effective_sqs_notification_queue_arn
...
module "app_tier" {
  ...
  sqs_notification_queue_arn      = local.effective_sqs_notification_queue_arn
  sqs_notification_queue_name     = local.effective_sqs_notification_queue_name
  sqs_notification_queue_url      = local.effective_sqs_notification_queue_url
}
