...
variable "frontend_image" {
  type    = string
  default = "docker.io/nimeshsv814/tf-frontend:v6.0.1"
}
...
variable "booking_service_image" {
  type    = string
  default = "docker.io/nimeshsv814/tf-booking-service:v6.0.1"
}
...
variable "create_sqs_notification_queue" {
  type        = bool
  description = "Create the SQS notification queue and dead-letter queue with Terraform"
  default     = true
}
...
variable "sqs_notification_queue_url" {
  type        = string
  description = "Existing SQS queue URL for notification events when create_sqs_notification_queue is false"
  default     = ""
}

variable "sqs_notification_queue_arn" {
  type        = string
  description = "Existing SQS queue ARN for notification events when create_sqs_notification_queue is false"
  default     = "arn:aws:sqs:us-east-1:*:smart-parking-notifications-queue"
}

variable "sqs_notification_visibility_timeout_seconds" {
  type        = number
  description = "Visibility timeout for notification SQS messages"
  default     = 60
}

variable "sqs_notification_message_retention_seconds" {
  type        = number
  description = "Retention period for notification SQS messages"
  default     = 345600
}

variable "sqs_notification_receive_wait_time_seconds" {
  type        = number
  description = "Long-poll wait time for notification SQS receives"
  default     = 10
}

variable "sqs_notification_max_receive_count" {
  type        = number
  description = "Number of failed receives before a notification message is sent to the DLQ"
  default     = 5
}
